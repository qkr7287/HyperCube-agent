...
### 완료
- **DockerCollector 클래스**: graceful degradation, Docker 미연결 시 30초마다 재시도, 복구 시 자동 감지
- **수집 타임아웃**: 고정 타임아웃 제거 → skip-if-busy guard 방식 (컨테이너 수에 무관하게 동작)
- **구조화된 로깅**: `src/logger.ts` — 타임스탬프/레벨/태그 형식, `LOG_LEVEL` 환경변수
- **대량 컨테이너 최적화**: 동시성 제한 `CONCURRENCY_LIMIT=10`, 개별 stats 타임아웃 3초
- **Command Handler (양방향 통신)**: Backend→Agent 명령 수신/응답 구현
  - `get_logs` — 컨테이너 로그 조회 (Docker stream header 파싱)
  - `inspect` — 컨테이너 상세 정보
  - `control` — start/stop/restart/pause/unpause/kill/remove
  - `system_info` — cpu_detail/processes/network_detail/users

### 메시지 프로토콜 (Backend ↔ Agent)
```
Backend → Agent: { type: "command", requestId, command, params }
Agent → Backend: { type: "command_response", requestId, success, data?, error? }
```

### 16번 서버 (Mac mini) 배포 완료
- 경로: `/home/agics-ai/github/HyperCube-agent`
- SSH: `root@192.168.0.16:2022`
- 20+ 컨테이너 환경에서 정상 동작 확인

### 남은 작업
- **토큰 로테이션**: 만료/갱신 메커니즘 (나중에)
- **테스트 코드**: 단위/통합 테스트 작성 (나중에)
- **배포 자동화**: CI/CD, 버전 태깅 (나중에)

### 결정사항
- 외부 로깅 라이브러리(Winston/Pino) 대신 경량 자체 로거 사용 (Agent는 경량 데몬)
- 고정 타임아웃 대신 skip-if-busy 패턴 채택 (컨테이너 수에 무관)
- Docker 소켓: Linux `/var/run/docker.sock`, Windows `//./pipe/docker_engine`

### 주의점
- `.env` 파일의 `DOCKER_SOCKET` 경로는 OS에 따라 다름
- Windows에서 첫 systeminformation 호출이 느림 (3~5초) → 첫 스냅샷은 타임아웃 없이 대기
- 16번 서버는 비밀번호 SSH만 가능 (sshpass 미설치, SSH 키 등록 안 됨)
